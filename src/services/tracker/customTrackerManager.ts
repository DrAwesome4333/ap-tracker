...
class CustomTrackerRepository implements ResourceRepository {
    ...
    constructor(locationManager: LocationManager, _inventoryManager: InventoryManager){
        this.#locationManager = locationManager;
        // this.#inventoryManager = inventoryManager;
        SaveData.getAllItems(DB_STORE_KEYS.customTrackersDirectory).then((directory: CustomTrackerDirectory) => {
            if(directory){
    ...
    #updateDirectory = (newDirectory: CustomTrackerDirectory) => {
        this.resources = Object.entries(newDirectory).map(([_uuid, manifest]) => manifest).flat();
    ...
    loadResource: (uuid: string, version: string) => Promise<Resource> = async (uuid, version) => {
        if(!this.#directory[uuid]) {
            throw new Error(`Failed to locate resource ${uuid}`);
        }
        // TODO support custom item trackers
        const resource = await SaveData.getItem(DB_STORE_KEYS.customTrackers, `${uuid}<${version}>`) as CustomLocationTrackerDef_V2;
        return new CustomLocationTracker(resource, this.#locationManager, customTrackerRepositoryUUID);
    }
    ...
    addTracker = (data: CustomLocationTrackerDef_V2 | CustomLocationTrackerDef_V1) => {
         ...
         const addTracker = () => {
            ...
            const trackerIndex = trackerVersions.map((manifest, index) => ({ manifest, index })).filter(({manifest}) => manifest.version === data.manifest.version)[0].index ?? -1;
            if(trackerIndex === -1){
                trackerVersions.push(data.manifest);
            } else {
                trackerVersions[trackerIndex] = data.manifest;
            }
            SaveData.storeItem(DB_STORE_KEYS.customTrackers, getTrackerKey(data.manifest));
